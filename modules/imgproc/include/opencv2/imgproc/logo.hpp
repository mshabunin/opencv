...
protected:
    ...
    inline Point getOrigin(Size imageSize, Location loc) const
    {
    ...
    inline void init(float scale)
    {
        ...
        CV_Assert_N(d1 > 0, d2 > 0, l > 0, d2 > d1, l >= d2);
        cb = getPoly(-60);
        cg = getPoly(0);
        cr = getPoly(-240);
...
public:
    ...
    {
        CV_Assert_N(factor > 0, !imageSize.empty());
    ...
    //! @param shift origin point for drawing (top left corner)
    void draw(InputOutputArray image, Point origin) const
    {
